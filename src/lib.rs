...
impl AAUnit {
    ...
    /// assume that output channels are interlaced
    #[inline]
    pub fn compute_two_two_non(&self, frames: usize, inputs: &[&[f32];2], outputs: &mut [&mut [f32];2]) -> Result<()> {
        // setup and copy input audio
        let [inputs0, inputs1] = inputs;
        let (inputs0, inputs1) = {
			let inputs0 = inputs0[..frames as usize].iter();
			let inputs1 = inputs1[..frames as usize].iter();
			(inputs0, inputs1)
		};
        let (wasm_inputs0, wasm_inputs1): (&mut [f32],&mut [f32]) = unsafe { 
            let bytes0 = 
        ...
        let zipped_inputs = inputs0.zip(inputs1).zip(wasm_inputs0).zip(wasm_inputs1);
        for (((input0, input1), wasm_input0), wasm_input1) in zipped_inputs {
            *wasm_input0 = *input0;
            *wasm_input1 = *input1;
        }
